...
function HamburgerDesplegable() {
  ...
  return (
    <div className="relative">
      ...
      >
        <Menu className="hover:scale-110 transition duration-200" />
      </div>

      {/* Sidebar Desplegable */}
      <motion.div
        initial={{ x: -480, opacity: 0 }}
        animate={{ x: isHovered ? 0 : -480, opacity: isHovered ? 1 : 0 }}
        transition={{ duration: 0.3 }}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="fixed left-0 top-0 h-full w-94 bg-gray-100 text-white shadow-lg p-5 sm:w-120"
      >
        <X className="block md:hidden cursor-pointer" onClick={() => setIsHovered(false)} />
